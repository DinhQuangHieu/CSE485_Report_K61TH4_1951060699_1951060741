...
// Tạo session: Mặc định mỗi phiên làm việc có thời hạn 24p
    session_start();
// login truyền Dữ liệu sang: Nhập dữ liệu từ login sang
   if(isset($_POST['btnSignIn'])){
       ...
       $pass = $_POST['txtPassword'];
        ...
        $sql = "SELECT * From taikhoan Where (Email = '$email' OR  TenDangNhap = '$email') AND MatKhau = '$pass'";
        $result = mysqli_query($conn,$sql);
        
        if(mysqli_num_rows($result) > 0){
            $_SESSION['isloginok'] = $email;
            header("location: afterlogin.php"); // chuyển hướng vô trang chủ quản trị
